...
module.exports = async (config, options, targetOptions) => {
  ...
  if (config.mode === 'development') {
    config.plugins.push(
      ...
      new WebpackNotifierPlugin({
        title: 'devops Backend App',
        contentImage: path.join(__dirname, 'logo-devops.png'),
      }),
    );
  }
      ...
      new BrowserSyncPlugin(
        {
          ...
          /*
          ghostMode: { // uncomment this part to disable BrowserSync ghostMode; https://github.com/devops/generator-devops/issues/11116
            clicks: false,
            location: false,
            forms: false,
            scroll: false,
  ...
  const patterns = [
    ...
    // devops-needle-add-assets-to-webpack - apppster will add/remove third-party resources in this array
  ];
  ...
  config.plugins.push(
    new webpack.DefinePlugin({
      ...
      // The root URL for API calls, ending with a '/' - for example: `"https://www.app.tech:8081/myservice/"`.
      // If this URL is left empty (""), then it will be relative to the current context.
      // If you use an API server, in `prod` mode, you will need to enable CORS
      // (see the `devops.cors` common apppster property in the `application-*.yml` configurations)
